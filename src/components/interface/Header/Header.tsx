...
export default function Header({ logo = personmodel, links }: HeaderProps) {
  ...
  return (
    <header className="w-full py-4">
      {/* justify between */}
      <div className="mx-3 md:mx-6 lg:mx-10 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link to="/" aria-label="Home">
            <img src={logo} alt="logo" className="w-10 h-10" />
          </Link>
          <a
            href="mailto:eliot.walter@amaris.com"
        ...
        <nav className="hidden md:flex items-center gap-6">
          {navLinks.map((l) => (
            <Link
              key={l.to}
              to={l.to}
              className="group relative inline-block text-sm font-medium hover:text-blue-400 focus-visible:outline-none"
            >
              {l.label}
